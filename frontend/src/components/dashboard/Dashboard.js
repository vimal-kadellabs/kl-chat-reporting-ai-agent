      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {statCards.map((card, index) => (
            <div
              key={index}
              className="bg-white/60 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 p-4 transition-all duration-300 hover:scale-105 hover:shadow-xl hover:bg-white/80 group"
            >
              <div className="flex items-start justify-between h-full">
                <div className="flex-1">
                  <p className="text-xs font-medium text-slate-500 mb-1 uppercase tracking-wider">{card.title}</p>
                  <p className="text-2xl font-bold text-slate-900 mb-1 group-hover:text-indigo-600 transition-colors duration-300">{card.value || 0}</p>
                  <p className="text-xs text-slate-400 leading-relaxed">{card.description}</p>
                </div>
                <div className={`w-12 h-12 ${card.bgColor} rounded-xl flex items-center justify-center shadow-md group-hover:shadow-lg`}>
                  <span className="text-lg opacity-80 group-hover:opacity-100">{card.icon}</span>
                </div>
              </div>
            </div>
          ))}
